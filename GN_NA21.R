...
GN_NA <- read.csv("./NA21_dat/samples/NA21_GEN20210728.csv")
GN_NA$datetime <- as.POSIXct(GN_NA$datetime, format = "%m/%d/%y %H:%M")

qplot(datetime, Results, data = GN_NA, geom="point") +
...
BW_Hobo <- subset(BW_Hobo, DateTime >= as.POSIXct("2021-07-28 12:32:00") & DateTime <= as.POSIXct("2021-07-28 14:35:00"))
...
sub_bg <- subset(BW_Hobo, DateTime >= as.POSIXct("2021-07-28 12:32:00") & DateTime <= as.POSIXct("2021-07-28 13:00:00")) #Lolomai
...
GN_NA$NO3_C <- (GN_NA$Results) 

#GB_NA[2,7] = 0.05
GN_NA$SpCond_C <- c(GN_NA$SpCond - bg_SpCond)
...
# write.csv(x = out, file = "./BTC_out/GEN_BTC_20210728.csv", row.names = TRUE)
