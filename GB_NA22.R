...
# Adjust the time range:
Hobo <- subset(Hobo, DateTime >= as.POSIXct("2022-10-03 12:46:00") & DateTime <= as.POSIXct("2022-10-03 13:50:00"))
...
## Reach morphology estimates:
##
sub_bg <- subset(Hobo, DateTime >= as.POSIXct("2022-10-03 12:46:00") & DateTime <= as.POSIXct("2022-10-03 12:50:00")) #Lolomai
bg_SpCond <- mean(sub_bg$SpCond)
## (2) Estimate conductivity slug based on mass of Cl added
...
peak_time <- Hobo[which.max(Hobo$SpCond),]$DateTime 
end_time <-as.POSIXct("2022-10-03 13:55:30")
time_diff_sec <- as.numeric(peak_time - inj_time)*60
time_tota_sec <- (as.numeric(end_time - inj_time)) * 3600 # minutes
...
# write.csv(x = out, file = "./BTC_out/GBL_NO3_BTC_BWL221003.csv", row.names = TRUE)
...
## Reach morphology estimates:
## (1) Determine the background conductivity
sub_bg <- subset(Hobo, DateTime >= as.POSIXct("2022-12-12 11:50:00") & DateTime <= as.POSIXct("2022-12-12 12:00:00")) #Lolomai
bg_SpCond <- mean(sub_bg$SpCond)
## (2) Estimate conductivity slug based on mass of Cl added
