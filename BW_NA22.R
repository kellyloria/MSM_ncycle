...
# ggsave(plot = BW_uptake, filename = paste("./figures/BWL220824.png",sep=""),width=4,height=7,dpi=300)
...
# Adjust the time range:
Hobo <- subset(BW_Hobo, DateTime >= as.POSIXct("2022-08-24 10:05:00") & DateTime <= as.POSIXct("2022-08-24 11:36:00"))

qplot(DateTime, Cond, data = BW_Hobo1, geom="point") +
  theme(axis.text.x = element_text(angle = 25, vjust = 1.0, hjust = 1.0))

## NH4 sample data ## 
dat <- read.csv("./NA22_dat/BWU_20220824/BWU20220824_NH4.csv")
dat$datetime <- as.POSIXct(paste(dat$date, dat$time), format = "%Y-%m-%d %H:%M:%S")
str(dat)
...
## Reach morphology estimates:
## (1) Determine the background conductivity
sub_bg <- subset(dat, datetime >= as.POSIXct("2022-08-24 10:05:00") & datetime <= as.POSIXct("2022-08-24 13:55:00")) #Lolomai
bg_SpCond <- mean(na.omit(sub_bg$ysi_SPC))
## (2) Estimate conductivity slug based on mass of Cl added
SpCond_mass <- c(2100*1500) # NOT sure
## Calculate Q
## Units = L/sec
dat_fl <-na.omit((dat[,c(6,8)]))
...
# Carboy concentrations 700g NaCl in 6L carboy
NaClmgL <- 700 * (1000) * (35.45/58.44) * (1/10)
carboy <- Nh4mgL/NaClmgL
...
# ggsave(plot = BW_uptake, filename = paste("./figures/BWU220824.png",sep=""),width=4,height=7,dpi=300)
