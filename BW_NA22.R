...
# Adjust the time range:
Hobo <- subset(Hobo, DateTime >= as.POSIXct("2022-10-12 10:50:00") & DateTime <= as.POSIXct("2022-10-12 13:00:00"))
...
## Reach morphology estimates:
## (1) Determine the background conductivity
sub_bg <- subset(Hobo, DateTime >= as.POSIXct("2022-10-12 10:50:00") & DateTime <= as.POSIXct("2022-10-12 11:00:00")) #Lolomai
bg_SpCond <- mean(sub_bg$SpCond)
## (2) Estimate conductivity slug based on mass of Cl added
...
# Carboy concentrations 300g KNO3 in 12L carboy
NO3_C <- 500 * (1000) * (62/101) *(1/12)
# Carboy concentrations 700g NaCl in 6L carboy
NaClmgL <- 700 * (1000) * (35.45/58.44) * (1/12)
carboy <- NO3_C/NaClmgL
...
#####################
## BWL 2022-12-19 ##
Hobo <-read.csv("./NA22_dat/BWL_20221219/BWL_221219_20775523_BOR.csv", skip=1)
summary(Hobo)
names(Hobo)
...
# Adjust the time range:
Hobo <- subset(Hobo, DateTime >= as.POSIXct("2022-12-19 12:40:00") & DateTime <= as.POSIXct("2022-12-19 14:01:00"))
...
## Reach morphology estimates:
## (1) Determine the background conductivity
sub_bg <- subset(Hobo, DateTime >= as.POSIXct("2022-12-19 12:40:00") & DateTime <= as.POSIXct("2022-12-19 13:00:00")) #Lolomai
bg_SpCond <- mean(sub_bg$SpCond)
## (2) Estimate conductivity slug based on mass of Cl added
...
inj_time <- as.POSIXct("2022-12-19 13:11:05") #Lolomai 
peak_time <- Hobo[which.max(Hobo$SpCond),]$DateTime 
end_time <-as.POSIXct("2022-12-19 14:01:00")
time_diff_sec <- as.numeric(peak_time - inj_time)*60
time_tota_sec <- (as.numeric(end_time - inj_time)) * 3600 # minutes
...
#####################
## NO3 sample data ##
dat <- read.csv("./NA22_dat/BWL_20221219/BWL_20221219_NO3.csv")
dat$datetime <- as.POSIXct(paste(dat$date, dat$time), format = "%Y-%m-%d %H:%M:%S")
str(dat)
